...
client = MongoClient('mongodb://test:test@13.125.251.107',27017)
db = client.dbjungle
...
def signup():
    ...
    try:
        ...
        curl=db.users.find_one({'id': payload['id']}, {'_id': False, 'id': False,'pw': False,'name_give': False, 'phone': False, 'dryingrack': False,'detergent': False, 'fever': False, 'hanger': False,'painkiller': False,'index': False})
...
@app.route('/find/canuse',methods=['POST'])
def use():
    name_use = request.form['name_give']
    a = db.users.find({'name_give':name_use},{'canuse':'O'})
    if a is not None:
        return jsonify({'result': 'fail', 'msg': '불가능합니다!'})
    else:
        db.users.update_one({'name_give':name_use},{"$set":{'canuse':"X"}})
        return jsonify({'result': 'success'})
